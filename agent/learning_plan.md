...
| 19 | **LangChain Fundamentals**: What is LangChain? PromptTemplate, ChatPromptTemplate, Messages (System/Human/AI), output parsers (StrOutputParser, PydanticOutputParser), **LCEL basics** (pipe operator | for chaining), simple chains | **Apply to Documents**: Document Loaders (PyPDFLoader, TextLoader), Text Splitters (RecursiveCharacterTextSplitter), **Semantic Chunking** basics. Parse first PDF | Longest Substring w/o Repeat | langchain_fundamentals.py |
...
| 21 | **Basic RAG Pipeline**: What is RAG? The full loop: chunk → embed → store in vector DB → retrieve similar chunks → generate answer with LLM. Build end-to-end RAG script using LangChain Chain + vector store retriever | **Advanced Retrieval**: Proposition Generation (decompose docs into atomic propositions) + Step-back Prompting (generate broader queries when results are sparse) | Binary Search | basic_rag_pipeline.py |
...
| 30 | **LangSmith Platform + RAG Eval Metrics**: What is LangSmith? Projects, traces, spans, datasets, annotators. Setting up LangSmith SDK. KV cache mechanics, PagedAttention, speculative decoding, INT8/INT4 quantization. Ragas (Faithfulness, Context Precision/Recall, Answer Relevancy). Traditional: F1, MAP, MRR, NDCG@K, BLEU, ROUGE | Setup LangSmith project. Build first trace. Build Ragas eval pipeline. Measure faithfulness + context precision | Level Order Traversal | langsmith_eval.py |
| 31 | **LangGraph Foundations**: What is LangGraph? StateGraph vs plain Chain. Defining state (TypedDict), Nodes (functions that modify state), Edges (conditional routing based on state values), Reducers (add_messages for conversation history). Build first 3-node linear graph | **PROJECT 1 START**: Apply LangGraph to Multi-Tenant Knowledge Agent. Build supervisor node → routes queries to vector DB / web search / conversation memory | Longest Consecutive Sequence | langgraph_foundations.py |
...
| 49 | **MCP Fundamentals + AI Observability**: What is MCP? Host/Client/Server architecture, JSON-RPC 2.0, Tools vs Resources vs Prompts. stdio vs HTTP SSE transport. Build first MCP server with one tool. **Then**: Monitoring latency p50/p95/p99, distributed tracing (Langfuse, LangSmith). Concept/data drift detection (Prometheus + Grafana). **MCP Threat Models**: Preventing Confused Deputy attacks, mTLS for server-to-server trust, prompt injection via tool output sanitization, containerized MCP server isolation. OAuth 2.1, OWASP MCP Top 10, per-client consent | Build first MCP server (filesystem tool). Then add security: mTLS + token exchange + output sanitization. Design monitoring dashboard | Serialize/Deserialize Tree | mcp_observability_security.py |
