...
| 13 | **Python Type Hinting** (typing module: Union, Optional, List, Dict) + **HTTP Protocol Fundamentals** (Request-Response lifecycle, GET vs POST, headers, status codes, MIME types) + **FastAPI intro** | Build first strictly-typed FastAPI endpoint with GET/POST that validates and serializes custom JSON payloads | Group Anagrams | fastapi_hello.py |
...
| 15 | **String methods, regex module** (re.search, re.findall, re.sub), JSON module + **Defensive Parsing**: handling markdown code fences, truncated brackets, unescaped quotes in LLM-like outputs | Build an error-resilient parser that strips markdown fences, extracts JSON blocks, handles malformed strings | Product of Array | defensive_json_parser.py |
| 16 | **LLM APIs**: OpenAI/Gemini, temperature, tokens, streaming. What is a token? Logit? Softmax? Autoregressive generation? | Build an interactive multi-turn Chat CLI with real-time text streaming and conversation history | Valid Palindrome | basic_llm_api.py |
...
| 22 | **Relational Database Fundamentals**: Tables, Rows, Primary/Foreign Keys, Data Types + **SQL Basics**: SELECT, INSERT, WHERE, JOIN + **Python DB connections** (sqlite3) | Build a basic SQLite database to store and retrieve document metadata | Search 2D Matrix | sql_fundamentals.py |
...
| 24 | **Hybrid Search Theory**: BM25 algorithm, sparse vs dense fusion, **Reciprocal Rank Fusion (RRF)** formula + pgvector HNSW vs IVFFlat tuning | Implement manual BM25 keyword search alongside dense search. Fuse results using RRF. Benchmark recall | Merge Two Sorted Lists | hybrid_search.py |
...
| Day | Morning (2hr) | Afternoon (2hr) | Evening DSA (1hr) | Deliverable |
|-----|--------------|-----------------|-------------------|------------|
| 25 | **Calculus for Deep Learning**: Derivatives, Partial Derivatives, Chain Rule + **Optimization Theory**: Gradient Descent, Learning Rates + **Linear Algebra**: Vector spaces, dot products, matrix multiplication, eigenvectors | Build a simple gradient descent optimizer in pure Python. Solve linear algebra practice problems | Reorder List | calculus_optimization.py |
| 26 | **Classical ML Fundamentals**: Logistic Regression, Random Forest, Gradient Boosting (XGBoost), K-Means, PCA. Study Ref: scikit-learn/scikit-learn | Implement each algorithm on toy datasets, compare performance. Probability & Statistics (Bayes, distributions) integrated into ML context | Max Depth Tree | classical_ml.py |
| 27 | **Classical NLP**: Bag of Words, N-grams, Tokenization + **Word Embeddings**: Word2Vec, GloVe (training vs loading pre-trained) + **Sequential Models**: RNN/LSTM theory, vanishing gradient problem | Train a basic Word2Vec embedding mapping. Visualize embedding space | Validate BST | nlp_embeddings.py |
| 28 | **Transformer Internals Deep Dive**: Why transformers? Sequence-to-Sequence bottlenecks. Multi-head attention math, positional encoding (RoPE), layer norm, residual connections, feed-forward networks. Study Ref: karpathy/minGPT | Implement a minimal transformer block from scratch. Compare with RNN forward pass | Invert Tree | transformer_block.py |
...
| Day | Morning (2hr) | Afternoon (2hr) | Evening DSA (1hr) | Deliverable |
|-----|--------------|-----------------|-------------------|------------|
| 32 | **Corrective RAG (CRAG)** + **Adaptive RAG** + **Multimodal AI Intro**: Vision-Language Models, OCR basics, layout-aware document parsing | Process a complex PDF using layout-aware chunking. Extract tables and charts. Build evaluator → web search fallback | Course Schedule | corrective_adaptive_rag.py |
...
| 34 | **Temporal Knowledge Graph Fundamentals**: Bi-temporal data modeling (valid time vs ingestion time), Entity resolution, Ontology creation via Pydantic models + **Cross-Session Memory**: Mem0 (semantic dedup) + Graphiti (temporal KGs). Study Ref: NirDiamant/Agent_Memory_Techniques | Build cross-session memory that tracks user preference changes over time with historical fact invalidation | Coin Change | agentic_rag_memory.py |
| 35 | **LLM-as-a-judge Eval Frameworks** + **DSPy Programmatic Signatures** (bootstrapping demonstrations) + Advanced Prompting (Tree of Thoughts, constrained decoding) | Build a self-evaluating LLM pipeline that grades its own outputs against heuristic rubrics | Longest Increasing Subsequence | llm_judge_dspy.py |
...
| 37 | **Containerization Basics**: Dockerfiles, Images, Containers, Port mapping, Volumes + **Cloud Infrastructure**: AWS ECR, ECS Task Definitions, IAM roles | Containerize the FastAPI/LangGraph app. Push to ECR. Deploy to AWS ECS Fargate | Task Scheduler | project1_deploy/ |
...
| 42 | **Storage Engine Fundamentals**: Row-oriented vs Columnar memory layouts, Apache Arrow architecture, zero-copy reads + **LanceDB**: S3-backed columnar scaling + **Multi-Tenant Vector Scaling**: Pre-filtering vs post-filtering recall collapse, hnsw.max_scan_tuples | Build a multi-tenant vector store using columnar caching. Contrast RAM scaling vs S3-backed | Best Time to Buy/Sell Stock | lancedb_multitenant.py |
...
| 45 | **Distributed Systems Architecture**: CAP Theorem, Consistency Levels (Strong/Bounded/Eventual), Sharding, Replication + **Milvus**: Knowhere, Proxy/QueryNode/IndexNode, GuaranteeTs, Clustering Compaction | Deploy and configure clustered Milvus. Measure read latency under different consistency settings | Course Schedule II | milvus_distributed.py |
...
| Day | Morning (2hr) | Afternoon (2hr) | Evening DSA (1hr) | Deliverable |
|-----|--------------|-----------------|-------------------|------------|
| 47 | **Advanced Networking Protocols**: WebSockets (persistent bi-directional TCP), Protocol Buffers (protobuf schema definitions), HTTP/2 multiplexing + **gRPC Streaming**: bidirectional streaming for token delivery | Build a high-throughput persistent bi-directional streaming connection for real-time token delivery | Find Min Rotated + Search in Rotated Array | streaming_protocols.py |
